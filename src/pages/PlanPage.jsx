...
export function PlanPage() {
  ...
  return <div className="page">
    ...
    <div className="space-y-5">
      {days.map((date, dayIndex) => <section key={date} className="soft-card overflow-hidden">
        ...
        <div className="grid gap-3 p-3 sm:grid-cols-3 sm:p-4">
          {mealTypes.map(mealType => { const entry = mealPlan.find(item => item.date === date && item.mealType === mealType); const recipe = allRecipes.find(item => item.id === entry?.recipeId); return <div key={mealType} className={`relative min-h-32 rounded-[1.35rem] border-2 border-ink/10 p-4 ${recipe ? mealAccent[mealType] : 'border-dashed bg-butter/70'}`}>
    ...
    <Modal open={Boolean(slot)} onClose={() => setSlot(null)} title={slot ? `${label(slot.mealType)} / ${formatDate(slot.date)}` : 'Pick a recipe'} size="large">
      ...
      <div className="grid gap-3 sm:grid-cols-2">{allRecipes.filter(recipe => recipe.mealType === slot?.mealType || slot?.mealType === 'dinner').map(recipe => <button key={recipe.id} type="button" onClick={() => { planRecipe(slot.date, slot.mealType, recipe.id); setSlot(null); }} className="flex items-center gap-3 rounded-2xl border-2 border-ink/10 bg-white p-3 text-left transition hover:scale-[1.02] hover:border-ink"><div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl"><RecipeCover recipe={recipe} /></div><span className="flex-1"><strong className="block font-display text-lg font-black">{recipe.name}</strong><small className="font-bold text-ink/45">{recipe.timeMinutes} min</small></span><ChevronRight size={18} /></button>)}</div>
    </Modal>
  </div>;
}
